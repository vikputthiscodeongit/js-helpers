...
## Compatibility
All helpers are fully compatible with the ECMAScript 2015 (ES6) specification except for that of `fetchWithTimeout`, which is an `async` function and thus requires an ECMAScript 2017 (ES8) compatible environment.

Keep in mind that these helpers were written with the intent to be used in a browser environment and thus they may utilize DOM methods.
